...
struct tcp_pcb * start_corr_stream(struct ip_addr* ipaddr, int port, vals_struct* vals)
{
	xil_printf("Sending the correlation results\n");
	return establish_connection(ipaddr, port, (void*)vals, send_corr_callback);
}

void send_corr_callback(void* _start_args, struct tcp_pcb *newpcb, err_t err){
	vals_struct * corr_vals = (vals_struct *) _start_args;
	int length = corr_vals->length;

	// message needs to include the "header" of CORR
	char *message = (char *)malloc(length + 1);
	xil_printf("In the corr callback\n");
	if(!message) {
		xil_printf("Failed to allocate message to send correlation values!\n");
	}

	// construct the message and send
	message[0] = CORR;
	for(int i = 0; i < length; ++i){
		message[i+1] = corr_vals->values[i];
	}
	microblaze_flush_dcache_range(message, length + 1);
	err = tcp_write(newpcb, (void*) message, length + 1, TCP_WRITE_FLAG_COPY);
	if (err |tcp_output(newpcb)) {
		xil_printf("send_corr_callback_failed\n");
	}
	free(message);
}
